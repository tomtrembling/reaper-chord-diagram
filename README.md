...
Open **Actions → Show action list** and search for `chord_diagram` to find them:

| Action | What it does | Needs a selection |
| --- | --- | --- |
| `chord_diagram.lua` — **Chord Diagram** | Opens the fretboard window on the selected item, to build or edit its chord | Exactly one empty item |
| `chord_diagram_regenerate.lua` — **Chord Diagram: regenerate missing diagrams** | Rebuilds every diagram image missing from the project, in one undo step | No |
| `chord_diagram_diagnostics.lua` — **Chord Diagram: copy diagnostics** | Copies a report to the clipboard for a bug report; changes nothing | No |

### The flow, once it has a hotkey

**Chord Diagram** is the one worth binding a key to — the other two are occasional. Select it in the
action list, press **Add…** under *Shortcuts for selected action*, and press the key you want.
...
ref/           the slice 002 spike, kept as the reference implementation. Not shipped.
